...
export const ccnaLayeredPrelude: NonNullable<CcnaLessonContent["teachingPrelude"]> = {
  terms: [
    ...
    { term: "Address and gateway", meaning: "An IP address identifies a network interface. With /24, the first three octets (numbers) identify the network: 192.168.1.x uses mask 255.255.255.0. A default gateway is the local router address used to reach another network. A MAC address identifies an Ethernet interface on a local link." },
...
export function ccnaLayeredVisual(): CcnaVisualStory {
  return {
    ...
    stages: [
      ...
      { title: "Check Router filtering", explanation: "ACL 199 blocks PC1's echo request at Router's LAN1 input; PC2 does not receive it or generate a reply. The rule does not deny replies arriving on LAN2. Compare counters and use the lab's ownership-checked rollback.", activeNodes: ["pc1", "switch", "router"], activeConnections: ["pc1-switch", "switch-router"], direction: "forward", sourceUrls: [refs.acl] }
    ]
  };
}
  ...
  return {
    ...
    steps: [
      ...
      consoleStep("Router", "Inspect and record the untouched router", "Start at > or #. Copy the complete output to Day4-baseline.txt. Require two unused routed ports, no existing ip access-group on either, and no ACL 199 anywhere. If any pre-existing setting is present, stop and choose a clean disposable router; do not remove or replace it. This lab targets Cisco IOS/IOS XE, not ASA or NX-OS. At privileged #, show ip access-lists lists IPv4 ACLs; if rejected, use show access-lists and check your image documentation. These are read-only inspections: do not create ACL 199 here. Stop if neither form supplies the required evidence; never change settings to force counters to appear.", [
        ...
        ["show ip access-lists", "List existing IPv4 ACL identifiers and rules. A numbered list uses an identifier such as 199, not a rule sequence number. If rejected, use show access-lists; do not create or change a list."]
      ...
      consoleStep("PC1", "Configure the source computer", "At PC1's VPCS prompt (PC1>), use the format ip address/prefix gateway. In the exact command below, 192.168.1.10 is PC1's address, /24 selects its mask and 192.168.1.1 is its gateway. Type the whole command, not the prompt, then press Enter. Do not enter Router IOS commands here.", [
      ...
      consoleStep("Router", "Save the clean pre-filter baseline", "Copy this output and both successful endpoint tests to Day4-baseline.txt. Confirm ACL 199 and interface ACL bindings are still absent. On this disposable router only, save the known working configuration; at Destination filename [startup-config]? press Enter. Never save the later fault configuration.", [
        ...
        ["show ip access-lists", "Check again that ACL 199 is unused before the exercise claims ownership of that identifier; use show access-lists if this form is rejected."],
      ...
      consoleStep("Router", "Apply the one lab-owned ICMP filter", "Continue only from the clean Step 7 baseline. If resuming after interruption, inspect running-config and ACLs first; never append into an existing 199. After the specific deny, permit ip any any permits other IPv4 traffic evaluated by this ACL; it does not create routes or services. Use this permissive teaching policy only in the fully disconnected lab, never on production or shared networks. Build the complete list before binding it inbound on mapped LAN1. The deny blocks only PC1's echo requests to PC2.", [
        ...
        ["show ip access-lists 199", "Before the next ping, save the deny match count and separately note your computer's clock time in Day4-baseline.txt. If rejected, use show access-lists and find list 199. Do not assume a missing count means zero; record unavailable."]
      ], "Only the test-owned list 199 is attached inbound on LAN1. Record the pre-test deny count or mark it unavailable before Step 9. Startup-config remains the clean baseline; interfaces and cables remain unchanged.", "Changing one narrowly scoped filter provides a known cause that can be checked with before-and-after rule counts and the identical peer test."),
      ...
      consoleStep("Router", "Detach and delete only test ACL 199", "Run only the four inspection commands first. Confirm LAN1 matches BOTH your baseline record and current show ip interface brief, and check its current inbound binding. Read all of running-config: 199 must have only our known LAN1 use. If any mapping or ownership is uncertain, stop before configure terminal; never paste this whole block blindly. The Step 7 baseline had no ACL attached. If 199 is not solely ours, stop. Detach our binding before deleting the list; leave any unexpected binding untouched.", [
        ["enable", "Return to Router privileged EXEC to begin explicit rollback of this lab-owned change."],
        ["show ip interface brief", "Match the current full LAN1 name, address and link to your recorded baseline and GNS3 cable endpoints. Stop on any mismatch."],
        ["show running-config", "Inspect the entire configuration for every reference to ACL 199. Stop if another interface or feature uses it; this lab owns only its LAN1 inbound binding."],
        ["show ip interface GigabitEthernet0/0", "Use the recorded LAN1 name. Confirm its inbound binding is our 199 and the original baseline had none. Stop here if uncertain; continue only after all three checks agree."],
        ...
        ["show ip access-lists", "Check that the test-owned list 199 is absent and unrelated lists have not been changed; use show access-lists if this form is rejected."],
...
export function ccnaLayeredBeginnerGuide(): NonNullable<CcnaLessonContent["beginnerGuide"]> {
  return {
    ...
    everydayComparison: {
      familiarSituation: "Imagine a parcel-delivery checklist. This is a memory aid, not a literal delivery process; real devices do not forward packets like parcels. Reading an address or asking about a reply organizes questions, not device actions.",
      networkMeaning: "In the real network, Switch handles Ethernet frames and Router selects an IP path and builds a new frame. PC2 receives the request and needs a return path. Each parcel comparison is an organizational metaphor, not a literal device action; our test uses ICMP, not postal delivery.",
    ...
    firstPractice: {
      task: "Using paper only, write PC1's own address and PC2's destination address beside your drawing. Trace both directions. Circle the planned filter and write two other possible causes of a timeout. Do not change a live network. Perform later tests only in the isolated lab setup, not on networks with production traffic or shared use.",
...
export function applyCcnaLayeredContract(content: Partial<CcnaLessonContent> = {}): CcnaLessonContent {
  // Every teaching, assessment and lab field is maintained as one sourced unit.
  // Retaining generated bibliography entries here can introduce unrelated claims.
  const sources = structuredClone(ccnaLayeredSources);
...
export const ccnaLayeredWritingBoundary = [
  ...
  "Introduce the first teaching section with this definition: A layered model is a diagram or checklist that organizes troubleshooting steps by grouping related network jobs. Immediately after introducing ANY mail/parcel analogy, state that it is a memory aid, not a literal delivery process. Each comparison must identify itself as an organizational metaphor, not a device action; prefer one short analogy over a misleading seven-part mapping.",
  "In the fault-evidence section AND workplace scenario, explicitly state that missed ping replies may involve host firewalls, link problems or rate limits. The real-world scenario is read-only analysis under authorization, not permission to remove workplace ACLs. Apply and roll back the deliberate fault only in the fully disconnected lab. Do not assert that other traffic was tested or works merely because a permit rule allows it.",
  "In the numbered-ACL removal quiz explanation, require detachment before no access-list 199, plus ownership and all-reference checks. This lab permits removal of only its single known LAN1 binding. If any other interface or feature references 199, stop, never detach unknown bindings. The full lab commands supply the required baseline/current-interface check before mutation. Preserve these checks in supporting practice answers too.",
  "Before any router installation, require legal permission specifically for that Cisco image in GNS3. Before the ACL configuration, explain that permit ip any any permits other IPv4 traffic evaluated after the narrow deny; the rule does not alone expose a network or create services. This teaching policy is only for the disconnected lab, not a production recommendation. Notes editors on Windows, Mac and Linux are separate from the GNS3 node Console. Repeat the isolation boundary when discussing rollback in practice and troubleshooting.",
