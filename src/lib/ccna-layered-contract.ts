...
export const ccnaLayeredPrelude: NonNullable<CcnaLessonContent["teachingPrelude"]> = {
  terms: [
    ...
    { term: "OSI and TCP/IP", meaning: "Open Systems Interconnection (OSI) describes physical, data link, network, transport, session, presentation and application jobs. TCP/IP groups these as link, internet, transport and application. Its name means Transmission Control Protocol/Internet Protocol." },
    ...
    { term: "Ping and ICMP", meaning: "Ping sends an Internet Control Message Protocol (ICMP) echo request and waits for an echo reply. No reply means the exchange did not complete before timeout, not proof of a particular failed layer. Host settings, firewalls, ICMP filters and either direction of the path can affect it." },
...
export function ccnaLayeredLab(): CcnaLessonContent["lab"] {
  return {
    ...
    setup: [
      "Complete the start-here guide and recap Day 2's console and gateway concepts. Use a new disconnected project. Keep any existing project untouched; save baseline text locally in a separate notes file. Never apply these ACL commands to a shared router.",
    ...
    steps: [
      step("Isolate and record the connection plan", "Create the new disposable project and connect the four named devices using the topology above. Keep Cloud, NAT and real network adapters out. Open a local notes file called Day4-baseline.txt and record each cable endpoint and the two router interface names before any edits.", [], "Four started nodes and exactly three links match the plan. Your existing projects are unchanged, and baseline notes identify this working copy.", "Isolation and an explicit record prevent an educational filter from changing a real service or an unrelated project."),
...
export function applyCcnaLayeredContract(content: CcnaLessonContent): CcnaLessonContent {
  ...
  return { ...content, sources, teachingPrelude: structuredClone(ccnaLayeredPrelude), visualStory: ccnaLayeredVisual(), lab: ccnaLayeredLab() };
}
