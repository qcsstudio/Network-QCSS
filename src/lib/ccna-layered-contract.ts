...
export function ccnaLayeredLab(): CcnaLessonContent["lab"] {
  return {
    ...
    steps: [
      ...
      consoleStep("PC1", "Configure the source computer", "At the VPCS prompt, configure PC1 and record its displayed address. VPCS accepts the prefix form below or ip 192.168.1.10 255.255.255.0 192.168.1.1. These set the same address, mask and gateway: use one form, type the whole line, then press Enter. Do not enter Router IOS commands here.", [
      ...
      consoleStep("PC2", "Configure the destination computer", "At the VPCS prompt, configure PC2 and record its displayed address separately from PC1. VPCS accepts the prefix form below or ip 192.168.2.10 255.255.255.0 192.168.2.1. Use one form, type the whole line, then press Enter.", [
      ...
      consoleStep("Router", "Apply the one lab-owned ICMP filter", "Continue only from the clean Step 7 baseline. If resuming after interruption, inspect running-config and ACLs first; never append into an existing 199. After the specific deny, permit ip any any permits other IPv4 traffic evaluated by this ACL; it does not create routes or services. Use this permissive teaching policy only in the fully disconnected lab, never on production or shared networks. Build the complete list before binding it inbound on mapped LAN1. The deny blocks only PC1's echo requests to PC2.", [
        ...
        ["show access-lists 199", "Before the next ping, save the deny rule's displayed match count and time in Day4-baseline.txt. Use show ip access-lists 199 if needed. Do not assume a missing count means zero; record unavailable if no count is shown."]
      ...
      consoleStep("Router", "Inspect the actual filter evidence", "Read the numbered list and compare its deny match count with the value recorded in Step 8. Subtract before from after: for example, 8 after minus 3 before means 5 new matches, not 8 from this test. Do not clear counters between readings. Presentation varies by image; if either count is unavailable, record that limitation instead of inventing a delta. If it does not increase, verify source, target and inbound port before attributing the loss to this filter.", [
        ...
        ["show access-lists 199", "Read the same deny rule after the peer tests; use show ip access-lists 199 if needed. Record its count and time beside Step 8's reading, then compare the two observations."],
