...
async function inspectVisual(
  ...
  const response = await openAIClient().responses.create({
    ...
    instructions: [
      ...
      "Any visible character-like marks on screens, documents, notebooks, labels, controls, hardware, or overlays are a blocking visible-text violation, including tiny, blurred, decorative, pseudo-code, or unreadable generated text. Abstract interfaces may use only non-character shapes, bars, light, or color blocks.",
