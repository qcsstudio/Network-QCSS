...
export function publicationIssues(post: BlogPost) {
  const issues = [...evaluateEditorialReadiness(post).issues];
...
export async function regenerateRadarContentPost(
  id: string,
  actor: string,
  briefOverride: Partial<Pick<BlogEditorialInput, "slug" | "topic" | "businessAngle" | "keywordCluster">> = {}
) {
