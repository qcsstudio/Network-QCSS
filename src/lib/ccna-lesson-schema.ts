...
export const ccnaLessonContentSchema = z.object({
  teachingPrelude: z.object({
    terms: z.array(z.object({ term: z.string().min(2).max(100), meaning: z.string().min(30).max(500) })).min(1).max(12),
    explanation: z.string().min(80).max(600),
    labBoundary: z.string().min(40).max(360)
  }).optional(),
...
export function ccnaOpenAIResponseSchema(allowedSourceUrls?: string[]) {
  ...
  // The reviewed topic prelude is supplied by the application before review, not generated by the writer.
  const schema = z.toJSONSchema(ccnaGeneratedLessonSchema.omit({ teachingPrelude: true }).extend({
    visualStory: ccnaVisualGenerationSchema.omit({ comparisons: true })
  }), { target: "draft-7" });
  // Rewrite the finished schema, not Zod's conversion callback: a $ref inserted
  // during draft-7 conversion can be wrapped in unsupported allOf clauses.
  function rewrite(value: unknown) {
    ...
    if (node.format === "uri") {
      ...
      if (allowed.length) {
        ...
        node.$ref = "#/$defs/verifiedSourceUrl";
      }
    } else Object.values(node).forEach(rewrite);
  }
  rewrite(schema);
  delete schema.$schema;
  if (allowed.length) schema.$defs = { ...schema.$defs, verifiedSourceUrl: { type: "string", maxLength: 1_000, enum: allowed } };
  return schema;
}
