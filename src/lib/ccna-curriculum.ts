...
const modules: ModuleSeed[] = [
  {
    ...
    topics: [
      ...
      ["OSI and TCP/IP models without memorization", "osi-and-tcp-ip-models", "Use layered models to organize fault evidence instead of merely reciting layer names.", "Foundation for 1.0", "Foundation for 2.4", "operations"],
      ...
      ["Copper, fiber, speed, duplex, and interface errors", "copper-fiber-speed-and-duplex", "Select media and diagnose physical-interface evidence.", "1.3-1.4", "1.1", "operations"],
      ["TCP and UDP through real applications", "tcp-versus-udp", "Choose the transport behavior that matches an application and read port evidence.", "1.5", "Supporting foundation", "operations"]
    ]
  },
  {
    id: "addressing",
    ...
    topics: [
      ["IPv4 addresses, masks, networks, and broadcasts", "ipv4-addressing-foundations", "Identify network, host, and broadcast boundaries from an IPv4 prefix.", "1.6", "1.3", "addressing"],
      ["Subnetting in a reliable four-step method", "ipv4-subnetting-four-step-method", "Calculate subnets, ranges, and broadcasts accurately under time pressure.", "1.6", "1.3", "addressing"],
      ["VLSM and an address plan that can grow", "vlsm-address-planning", "Allocate unequal subnets without overlap and leave intentional growth space.", "1.6", "1.3", "addressing"],
      ["Public, private, APIPA, and default gateways", "ipv4-address-types-and-gateways", "Recognize address scope and diagnose wrong-gateway symptoms.", "1.6-1.7", "1.3", "addressing"],
      ["IPv6 notation, prefixes, and compression", "ipv6-notation-and-prefixes", "Read, shorten, expand, and compare IPv6 prefixes correctly.", "1.8", "1.4", "addressing"],
      ["IPv6 unicast, multicast, anycast, and link-local", "ipv6-address-types", "Select the correct IPv6 address type and identify its scope.", "1.9", "1.4 (unicast); other types support understanding", "addressing"],
      ["SLAAC, modified EUI-64, and IPv6 neighbor discovery", "slaac-eui64-and-neighbor-discovery", "Explain how a host forms an address and discovers its local gateway.", "1.8-1.9", "1.4", "addressing"]
    ]
  },
  {
    id: "switching",
    ...
    topics: [
      ...
      ["MAC learning, flooding, forwarding, and aging", "ethernet-switching-and-mac-learning", "Predict a switch decision from source and destination MAC information.", "1.13", "Foundation for 2.1-2.4", "switching"],
      ["VLANs and access ports", "vlans-and-access-ports", "Verify data and voice VLANs and edge-port settings for computers, APs, phones and virtualized hosts; separate real PoE observations from simulated switching.", "1.1.h, 2.1", "2.2", "switching"],
      ...
      ["PortFast, BPDU guard, root guard, and loop guard", "spanning-tree-protection", "Apply Layer 2 protections at the correct trust boundary; distinguish BPDU filtering in the v1.1 blueprint.", "2.5", "2.5", "security"],
      ["Troubleshooting a switched campus path", "switched-campus-troubleshooting", "Correlate interface, VLAN, MAC, LACP and spanning-tree evidence with show commands, extended ping, traceroute and packet-capture output.", "2.1-2.5", "2.1-2.5", "switching"]
    ]
  },
  {
    id: "wireless",
    ...
    topics: [
      ["RF, bands, channels, interference, and coverage", "wireless-rf-channels-and-interference", "Explain why signal quality depends on more than visible bars.", "1.11", "1.5", "wireless"],
      ...
      ["Wired and wireless client troubleshooting", "wireless-client-troubleshooting", "Inspect Windows, macOS and Linux IP settings and reachability; separate RF, authentication, DHCP, DNS and upstream routing evidence.", "1.10-1.11, 2.6-2.9", "1.6", "wireless"]
    ]
  },
  {
    id: "routing",
  ...
  },
  {
    ...
    topics: [
      ...
      ["NTP and why trustworthy time matters", "ntp-and-network-time", "Configure time synchronization and connect clock accuracy to logs and certificates.", "4.2", "Supporting operations; not a named objective", "operations"],
      ["QoS classification, marking, queuing, and congestion", "qos-foundations", "Explain how delay, jitter, and loss shape application experience, including policing and shaping.", "4.7", "Supporting operations; not a named objective", "operations"],
      ...
      ["Secure administration with SSH, SFTP and SCP", "ssh-scp-and-secure-administration", "Protect administration and transfer configuration or software with SFTP/SCP; contrast legacy FTP/TFTP capabilities without recommending them for sensitive files.", "4.8-4.9", "4.2", "security"]
    ]
  },
  {
    id: "security",
    ...
    topics: [
      ...
      ["Standard IPv4 ACLs", "standard-ipv4-acls", "Place and verify numbered and named source-only lists without unintended denial.", "5.6", "4.6", "security"],
      ["Extended IPv4 ACLs", "extended-ipv4-acls", "Filter protocol, source, destination and port with numbered and named lists and deliberate placement.", "5.6", "4.6", "security"],
      ...
      ["Storm control and IPv6 RA guard", "storm-control-and-ra-guard", "Limit disruptive Layer 2 traffic and block rogue IPv6 router advertisements.", "Extension beyond 5.7", "4.7", "security"],
  ...
  },
  {
    ...
    topics: [
      ["Virtual machines, containers, and cloud network boundaries", "virtualization-containers-and-cloud-networks", "Explain hypervisors, virtual machines, containers and VRFs; separate virtual workload paths from public-cloud features that the lab does not reproduce.", "1.12", "1.2; VRFs are supporting context", "operations"],
      ...
      ["REST APIs, CRUD, authentication, and HTTP evidence", "rest-api-foundations", "Read a request and response safely without guessing from a status code alone.", "6.5", "Supporting automation; not a named objective", "automation"],
      ["JSON for network engineers", "json-for-network-engineers", "Navigate objects, arrays, names, and values in operational API output.", "6.7", "Supporting automation; not a named objective", "automation"],
      ["Ansible and Terraform: configuration versus state", "ansible-and-terraform", "Execute and verify bounded lab commands with Ansible; explain Terraform and infrastructure as code separately, including state and rollback limits.", "6.6", "5.3, 5.5", "automation"],
...
export const ccnaCourseFacts = {
  ...
  researchHorizon: "2027-09-15",
  revision: "ccna-2026-09-15",
...
export function ccnaExamStatus(now = new Date()) {
  const launched = now.toISOString().slice(0, 10) >= ccnaCourseFacts.nextLaunchDate;
  return { activeVersion: launched ? ccnaCourseFacts.nextVersion : ccnaCourseFacts.currentVersion,
    transition: launched ? "v2.0 launched 3 February 2027" : "v1.1 through 2 February 2027; v2.0 from 3 February 2027",
    verificationDue: now.getTime() - Date.parse(ccnaCourseFacts.verifiedAt) > 30 * 86_400_000 };
}
