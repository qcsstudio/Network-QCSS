...
import { nextCcnaPublicationJob, runCcnaDailyEdition } from "@/lib/ccna-learning";
...
export async function GET(request: Request) {
  ...
  try {
    const queued = await nextCcnaPublicationJob();
    if (queued) {
      const result = await runCcnaPublicationJob(queued.id);
      return NextResponse.json({ ok: true, result, distributionId: "distributionId" in result ? result.distributionId : "" }, { headers: noStoreHeaders });
    }
    if (new URL(request.url).searchParams.get("queuedOnly") === "1") {
      return NextResponse.json({ ok: true, result: { action: "not_due", reason: "No requested CCNA publication job is due." } }, { headers: noStoreHeaders });
    }
    ...
    console.error("CCNA daily automation failed.", { message: error instanceof Error ? error.message : "Unknown error" });
