...
export async function POST(request: Request) {
  ...
  const body = parsed.data as { contentId?: unknown; excludeContentIds?: unknown; force?: unknown; limit?: unknown };
  const limit = typeof body.limit === "number" ? Math.max(1, Math.min(Math.floor(body.limit), 5)) : 1;
  const force = body.force === true;
  const contentId = typeof body.contentId === "string" && body.contentId.length <= 160 ? body.contentId.trim() : "";
  ...
  await createAuditLog(
    {
      ...
      metadata: { contentId: contentId || null, excluded: excludeContentIds.length, force, limit, outcomes }
    },
