  ...
  return (
    <main className="verifygrid-report-page portal-report-page">
      ...
      <article className="verifygrid-report-document">
        ...
        <section className="verifygrid-report-integrity"><div><span>Released</span><strong>{report.releasedAt?.toLocaleString() || report.generatedAt.toLocaleString()}</strong></div><div><span>Scope hash</span><strong>{report.scopeHash}</strong></div><div><span>Snapshot SHA-256</span><strong>{report.snapshotSha256}</strong></div><div><span>Integrity chain</span><strong>{text(report.chainHash)}</strong></div><div><span>Digital signature</span><strong>{report.signature ? `${text(report.signatureAlgorithm)} | key ${text(report.signingKeyId)}` : "Not available"}</strong></div></section>
        <section className="verifygrid-report-section"><h2>Assurance summary</h2><div className="verifygrid-report-metrics"><div><span>In-scope targets</span><strong>{count(summary.inScopeTargets)}</strong></div><div><span>Findings</span><strong>{count(summary.findings)}</strong></div><div><span>Critical / high</span><strong>{count(summary.critical)} / {count(summary.high)}</strong></div><div><span>Known exploited</span><strong>{count(summary.knownExploited)}</strong></div><div><span>Closed</span><strong>{count(summary.closed)}</strong></div><div><span>Imported observations</span><strong>{count(summary.importedObservations)}</strong></div><div><span>Methodology tests</span><strong>{count(summary.methodologyTests)}</strong></div><div><span>Tests complete</span><strong>{count(summary.methodologyCompleted)}</strong></div><div><span>Tests with findings</span><strong>{count(summary.methodologyWithFindings)}</strong></div></div></section>
        <section className="verifygrid-report-section"><h2>Methodology coverage</h2><p>Service-specific tests and analyst conclusions captured in this report version.</p><div className="verifygrid-report-table" role="table" aria-label="Methodology coverage"><div className="verifygrid-report-row verifygrid-report-methodology-row header" role="row"><span>Test</span><span>Standard</span><span>Status</span><span>Result</span></div>{methodology.length ? methodology.map((testCase, index) => <div className="verifygrid-report-row verifygrid-report-methodology-row" key={`${text(testCase.code)}-${index}`} role="row"><strong>{text(testCase.code)}<small>{text(testCase.title)}</small></strong><span>{text(testCase.standardRef)}</span><span>{text(testCase.status)}</span><span>{text(testCase.resultSummary, "No conclusion recorded")}</span></div>) : <p className="verifygrid-report-empty">No methodology tests were present in this snapshot.</p>}</div></section>
