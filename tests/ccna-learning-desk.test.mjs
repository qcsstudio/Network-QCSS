    ...
    for (const width of [320, 390, 768, 1440]) {
      ...
      await page.route("**/*", async (route) => {
        ...
        if (request.url() === "http://ccna-qa.test/api/admin/ccna-lessons") {
          requests += 1;
          assert.equal(request.postDataJSON().id, lesson.id);
          if (requests === 1) {
            assert.equal(request.postDataJSON().action, "draft");
            return route.fulfill({ contentType: "application/json", body: JSON.stringify({ ok: true, lessons: [{ ...lesson, status: "draft" }] }) });
          }
      ...
      await page.getByRole("button", { name: "Generate lesson", exact: true }).waitFor();
      assert.equal(requests, 1, "Restoration must not generate, publish or queue LinkedIn.");
      assert.equal(await page.getByRole("button", { name: "Restore to draft", exact: true }).count(), 0);
      ...
      assert.equal(requests, 2, "The UI must not silently start another paid generation.");
