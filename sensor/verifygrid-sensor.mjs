...
const VERSION = "1.0.0";
const ALLOWED_CAPABILITIES = new Set(["asset_inventory", "dns_posture", "tls_posture", "tcp_service_validation", "http_security_headers"]);
...
function targetEndpoint(target) {
  ...
  if (type === "url" || /^https?:\/\//i.test(value)) {
    ...
    return { identifier: value, host: url.hostname, port: Number(url.port || (url.protocol === "https:" ? 443 : 80)), url };
  }
  ...
  return { identifier: value, host, port: 443, url: new URL(`https://${net.isIPv6(host) ? `[${host}]` : host}/`) };
}
...
async function runTarget(capability, target) {
...
function validateJob(job) {
  ...
  if (!ALLOWED_CAPABILITIES.has(manifest.capability?.id) || manifest.capability?.humanApprovalRequired) throw new Error("The manifest capability is not available to automated sensors.");
...
async function handleJob(job) {
  ...
  const heartbeat = setInterval(() => api("/api/verifygrid/sensor/jobs/heartbeat", { jobId: job.id, state: "running", version: VERSION, region }).catch(() => {}), Math.max(15, Number(job.heartbeatSeconds || 60)) * 1000);
  try {
    await api("/api/verifygrid/sensor/jobs/heartbeat", { jobId: job.id, state: "running", version: VERSION, region });
    const observations = [];
    ...
    for (const targetValue of manifest.targets) {
      ...
      try {
        observations.push(await runTarget(manifest.capability.id, target));
      } catch (error) {
    ...
    await api("/api/verifygrid/sensor/jobs/failure", { jobId: job.id, manifestSha256: job.manifestSha256, error: message, retryable: true }).catch(() => {});
...
process.stdout.write(`QCS VerifyGrid sensor ${VERSION} started in ${region}.\n`);
